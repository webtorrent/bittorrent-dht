...
function DHT (opts) {
  ...
  this._fingerprint = opts.fingerprint || sha1
...
DHT.prototype._put = function (opts, cb) {
  ...
  var key = isMutable
    ? this._fingerprint(opts.salt ? Buffer.concat([opts.salt, opts.k]) : opts.k)
    : this._fingerprint(bencode.encode(v))
...
DHT.prototype.get = function (key, opts, cb) {
  ...
  var _fingerprint = this._fingerprint
  function onreply (message) {
    ...
    if (isMutable) {
      ...
      if (equals(_fingerprint(r.salt ? Buffer.concat([r.salt, r.k]) : r.k), key)) {
        value = r
        return false
      }
    } else {
      if (equals(_fingerprint(bencode.encode(r.v)), key)) {
        value = r
        return false
      }
    }
...
DHT.prototype._onput = function (query, peer) {
  ...
  var key = isMutable
    ? this._fingerprint(a.salt ? Buffer.concat([a.salt, a.k]) : a.k)
    : this._fingerprint(bencode.encode(v))
