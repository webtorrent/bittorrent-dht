...
class DHT extends EventEmitter {
  constructor (opts = {}) {
    ...
    const ping = (opts, cb) => {
      ...
      this._debug('received ping', older)
      this._checkNodes(older, false, (_, deadNode) => {
        if (deadNode) {
          this._debug('swaping dead node with newer', deadNode)
          swap(deadNode)
          return cb()
        }

        this._debug('no node added, all other nodes ok')
        cb()
      })
    }

    const bootstrap = () => {
      if (!this.destroyed) this._bootstrap(opts.bootstrap !== false)
    }

    const onlistening = () => {
      this.listening = true
      this._debug('listening %d', this.address().port)
    ...
    this._rpc.on('error', this.emit.bind(this, 'error'))
    this._rpc.on('listening', onlistening)
    this._rotateSecrets()
    this._verify = opts.verify || null
    this._host = opts.host || null
    this._interval = setInterval(this._rotateSecrets.bind(this), ROTATE_INTERVAL)
  ...
  _setBucketCheckInterval () {
    ...
    const checkBucket = () => {
      const diff = Date.now() - this._rpc.nodes.metadata.lastChange

      if (diff < this._bucketOutdatedTimeSpan) return queueNext()

      this._pingAll(() => {
        if (this.destroyed) return

        if (this.nodes.toArray().length < 1) {
    ...
    const queueNext = () => {
      if (!this._runningBucketCheck || this.destroyed) return
      const nextTimeout = Math.floor(Math.random() * interval + interval / 2)
      this._bucketCheckTimeout = setTimeout(checkBucket, nextTimeout)
    }
  ...
  announce (infoHash, port, cb = noop) {
  ...
  _preannounce (infoHash, port, cb) {
    this.lookup(infoHash, err => {
      if (this.destroyed) return cb(new Error('dht is destroyed'))
      if (err) return cb(err)
      this.announce(infoHash, port, cb)
    })
  }

  lookup (infoHash, cb = noop) {
    infoHash = toBuffer(infoHash)
    let aborted = false
    ...
    const emit = (values, from) => {
      if (!values) values = this._peers.get(infoHash.toString('hex'), 100)
      const peers = decodePeers(values)
      for (const peer of peers) {
        this.emit('peer', peer, infoHash, from || null)
      }
    }
  ...
  _bootstrap (populate) {
    ...
    this._rpc.populate(this._rpc.id, {
      q: 'find_node',
      a: {
        id: this._rpc.id,
        target: this._rpc.id
  ...
  _closest (target, message, onmessage, cb) {
    ...
    const onreply = (message, node) => {
      ...
      if (message.r.token && message.r.id && Buffer.isBuffer(message.r.id) && message.r.id.length === this._hashLength) {
        this._debug('found node %s (target: %s)', message.r.id, target)
        table.add({
          id: message.r.id,
          host: node.host || node.address,
          port: node.port,
    ...
    const done = (err, n) => {
      if (err) return cb(err)
      this._tables.set(target.toString('hex'), table)
      this._debug('visited %d nodes', n)
      cb(null, n)
    }
